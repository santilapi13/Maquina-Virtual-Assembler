...
int isASym(char cad[],TSym simbolos[],int Nsym);
int decSym(char cad[],TSym simbolos[],int Nsym);
...
void decodifica(char **parsed,int nInst, TReg tablaMnem[],TSym simbolos[],int *cantOp,int *codOp,int *tipoOpA,int *tipoOpB,int *opA,int *opB,int NtablaMnem,int Nsym,TErr errores[],int *Nerr,char tablaReg[][MAXV]);
    ...
    for (i=1;i<argc;i++)
        if (!strcmp(argv[i],"-o"))
            *outputOn = 0;
            ...
            else if (!strcmp(extFile,".mv1") || !strcmp(extFile,".mv2"))
...
int isASym(char cad[],TSym simbolos[],int Nsym) {
    int i=0;
    char simbolo[MAXV];
    codUpper(cad,simbolo);
    while (i<Nsym && strcmp(simbolo,simbolos[i].sym))
        i++;
    return i<Nsym && !strcmp(simbolo,simbolos[i].sym);
}
...
void procesa(char **parsed,TReg tablaMnem[],int Ntabla,TSym simbolos[],int *Nsym,int *nInst,TErr errores[],int *Nerr,TStr tablaStr[],int *Nstr) {
    char mnem[6],simbolo[MAXSYM],*out;
    int i;
    ...
    if (parsed[0]) {    // Si tiene rótulo
        ...
        while (i<*Nsym && strcmp(simbolo,simbolos[i].sym))
            i++;
        if (i >= *Nsym) {
            if (strlen(simbolo)>=3 && !(simbolo[0]>='0' && simbolo[0]<='9')) {
...
int isAReg(char cad[],char tablaReg[][MAXV]) {
    ...
    while (i<CANT_REG && strcmp(aux,tablaReg[i]))
        i++;
...
int decOpInd(char cad[],char tablaReg[][MAXV],TSym simbolos[],int Nsym) {
    ...
    if (cad[i] == '+' || cad[i] == '-') {
        ...
        while (cad[i] == ' ')
            i++;
        offset = decOpInm(cad+i,simbolos,Nsym)*signo;
    }
    ...
    if (*cantOp != 0) {
        if (isAReg(parsed[2],tablaReg))
            *tipoOpA = 1;           // A de registro
            ...
            if (isInmed(parsed[2]+1) || isASym(parsed[2]+1,simbolos,Nsym))
                *tipoOpA = 2;       // A directo
            else
        ...
        if (*cantOp == 2){
            if (isAReg(parsed[3],tablaReg))
                *tipoOpB = 1;       // B de registro
                ...
                if (isInmed(parsed[3]+1) || isASym(parsed[3]+1,simbolos,Nsym))
                    *tipoOpB = 2;   // B directo
                else
...
void codUpper(char *cod,char codOp[]) { // Pasa string a mayuscula
    unsigned int i=0;
    while (cod[i] != '\0' && cod[i] != ' ' && cod[i] != '\t' && cod[i] != ']') {
