...
public class MaquinaVirtual {
	...
	private ALU alu;
	private ArrayList<UnidadDisco> discos = new ArrayList<UnidadDisco>();
	private static MaquinaVirtual instancia = null;
	
	public static MaquinaVirtual getInstancia() {
        if (instancia == null)
            instancia = new MaquinaVirtual();
        return instancia;
    }
	...
	public void sys() throws InterruptedException, IOException {
		this.alu.sys(0x1000000F); // se fuerza el valor de un sys %F y el 1 es para evitar que se limpie la
									// pantalla
	}
	...
	public void cargaDiscos(String args[]) {
		int i;

		for (i = 0; i < args.length; i++) {
			if (args[i].endsWith(".vdd") && (discos.size() < 256)) // Limite max de discos = 255
				this.discos.add(new UnidadDisco(args[i])); // Se le pasa como parametro el archivo asi creo el objeto
															// disco
			else if (discos.size() > 255) {
				System.out.println("Se alcanzo el limite de discos (255)");
				break;
			}
	...
	public ArrayList<UnidadDisco> getDiscos() {
		return discos;
	}
