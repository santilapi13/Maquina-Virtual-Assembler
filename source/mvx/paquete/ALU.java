...
// LLAMA A LA INSTRUCCION A EJECUTAR
...
// INSTRUCCIONES 2 OPERANDOS (Parte I)
...
// INSTRUCCIONES 1 OPERANDO (Parte I)
...
// INSTRUCCIONES SIN OPERANDOS (Parte I)
	public void stop() {
		System.exit(0);
	}

// INSTRUCCIONES STRINGS (Parte II)
	public void smov(int topA, int topB, int vopA, int vopB) {
		...
		while (b != '\0') {
			if (topA == 2) // Directo
				...
				Memoria.getInstancia().modificaRAM(validarSegmento((cont << 4) + vopA), b);
			}
			cont++;
			if (topB == 3)
				b = valor2(topB, (cont << 4) + vopB);
			else
...
// INSTRUCCIONES SUBRUTINAS (Parte II)
	...
	public void breakParametroB() {
		...
		if (cod == 1)
			...
			if (cod == 3) {
				...
				System.out.print("%");
				System.out.format("%08X  %d \n", Memoria.getInstancia().getValorRAM(i),
						Memoria.getInstancia().getValorRAM(i), (Memoria.getInstancia().getValorRAM(i)));
			} else if (cod == 4) {
				String op1 = primeraDireccion(aux), op2 = segundaDireccion(aux);
				int i = Integer.parseInt(op1), j = Integer.parseInt(op2);
				for (int t = i; t <= j; t++) {
					System.out.format("[%04d] ", t);
					System.out.print("%");
					System.out.format("%08X  %d \n", Memoria.getInstancia().getValorRAM(t),
							Memoria.getInstancia().getValorRAM(t));
				}
			} else if (cod == 5) {
			}
...
// METODOS LECTURA Y ESCRITURA
	public void lectura() {
		String aux;
		boolean prompt = ((Registros.getInstancia().getEAX() >> 11) & 0x1) == 0,
				saltoLinea = ((Registros.getInstancia().getEAX() >> 8) & 0x1) == 0;
		int a, comienzo, cantidad, cont, auxInt;
		comienzo = (Registros.getInstancia().getEDX() & 0xffff) + inicioSegmento(Registros.getInstancia().getEDX());
	...
	public void lecturaString() {
		int pos = (Registros.getInstancia().getEDX() & 0xfff) + inicioSegmento(Registros.getInstancia().getEDX()),
				q = 0;
		char valor;
		boolean promt = (((Registros.getInstancia().getEAX() >> 11) & 0x1) == 0);
		String aux;
		Scanner leer = new Scanner(System.in);
		if (promt)
	...
	public void escrituraString() {
		...
		boolean promt = ((Registros.getInstancia().getEAX() >> 11) & 0x1) == 0,
				endline = ((Registros.getInstancia().getEAX() >> 8) & 0x1) == 0;
		if (promt)
			System.out.format("[%d]", pos);
		while (Memoria.getInstancia().getValorRAM(pos) != '\0')
			System.out.format("%c", Memoria.getInstancia().getValorRAM((pos++)));
		if (endline)
			System.out.print("\n");
	}
...
// METODOS DISCOS
	...
	public void lecturaDisco(UnidadDisco disco, int cilindro, int cabeza, int sector, int inicioBuffer) {
		...
		try {
			...
			int i, tamanoSector = disco.getTamanoSector();

			int tamañodisco = disco.getCilindros() * disco.getCabezas() * disco.getSectores() * tamanoSector;

			int desplazamientoBytes = 512 + cilindro * disco.getCilindros() * disco.getSectores() * tamanoSector
					+ cabeza * disco.getSectores() * tamanoSector + sector * tamanoSector;

			RandomAccessFile archivo = disco.getArchivo();

			int offsetBuffer = inicioBuffer & 0xFFFF; // En celdas
			int segmentoBuffer = inicioBuffer >> 16 & 0xFFFF; // 0 si el buffer estara en el DS, 2 si estara en el ES
			...
			int tamanoNecesarioArch = desplazamientoBytes + cantsectores * tamanoSector;

			// Analizo si debo extender el archivo (en caso de que se puedan leer todos los
			// sectores)
			if ((tamanoNecesarioArch > archivo.length() && tamanoNecesarioArch <= tamañodisco)
					|| (tamanoNecesarioArch <= archivo.length())) { // Debo extender el archivo si, cuando lo extiendo,
				...
				if (tamanoNecesarioArch > archivo.length() && tamanoNecesarioArch <= tamañodisco) // Si debo extender el
																									// archivo
					archivo.setLength(desplazamientoBytes + cantsectores * tamanoSector);

				// Analizo desde el lado del buffer

				// analizo si entran todos en el buffer
				if (offsetBuffer + cantsectores * tamanoSector / 4 <= tamañoSegmento) { // Si se cumple -> hay lugar
																						// para todos en el buffer
					...
					archivo.seek(desplazamientoBytes);
					for (i = 0; i < (cantsectores * tamanoSector / 4); i++) { // leo de a celdas
				...
				int cantSectoresPosibles = (int) (tamañodisco - desplazamientoBytes) / tamanoSector;
				if (cantSectoresPosibles > 0) { // analizo si puedo leer y pasar al menos 1
					...
					if (offsetBuffer + cantSectoresPosibles * tamanoSector / 4 <= tamañoSegmento) { // Si se cumple ->
																									// hay lugar para
																									// todos en el
																									// buffer
						// hago la lectura del archivo al buffer
						...
						int lectura;
						archivo.seek(desplazamientoBytes);
						for (i = 0; i < (cantSectoresPosibles * tamanoSector / 4); i++) { // leo de a celdas
							lectura = archivo.readInt();
							Memoria.getInstancia().modificaRAM(inicioAbsolutoBuffer + i, lectura); // celda , nuevoValor
						}
	...
	public void escribirDisco(UnidadDisco disco, int cilindro, int cabeza, int sector, int inicioBuffer) {
		...
		try {
			...
			int tamanoSector = disco.getTamanoSector();
			...
			int regSegmento = Registros.getInstancia().getReg(segmentoBuffer); // tendre el DS o el ES
			int inicioSegmento = regSegmento & 0XFFFF;
			int tamanoSegmento = regSegmento >> 16 & 0XFFFF; // tamaño en celdas

			if (offsetBuffer + (cantsectores * tamanoSector / 4) <= tamanoSegmento) { // Verifico si "hay lugar" en el
				...
				if (tamanoNecesarioArch <= tamañodisco) { // hay lugar para pasar todas las celdas

					if (tamanoNecesarioArch < archivo.length())
						archivo.setLength(tamanoNecesarioArch);
					...
					archivo.seek(desplazamientoBytes); // El SEEK extiende el archivo en caso de ser necesario

					for (i = 0; i < (cantsectores * tamanoSector / 4); i++) { // cantidad de celdas que voy a pasar del
																				// buffer al archivo
					...
					int cantSectoresPosibles = (int) (tamañodisco - desplazamientoBytes) / tamanoSector;
					if (cantSectoresPosibles > 0) { // verifico si puedo pasar al menos 1 sector

						int inicioAbsolutoBuffer = inicioSegmento + offsetBuffer;
						int i, valoraescribir;

						if (desplazamientoBytes + cantSectoresPosibles * tamanoSector > archivo.length())
							archivo.setLength(desplazamientoBytes + cantSectoresPosibles * tamanoSector);

						// Hago el paso de buffer a archivo
						archivo.seek(desplazamientoBytes);

						for (i = 0; i < (cantSectoresPosibles * tamanoSector / 4); i++) { // cantidad de celdas que voy
																							// a pasar del buffer al
																							// archivo
							valoraescribir = Memoria.getInstancia().getValorRAM(inicioAbsolutoBuffer + i);
							archivo.writeInt(valoraescribir);
						}
						...
						Registros.getInstancia().setAH(0xFF);
						// No hay lugar para escribir ningun sector
					}

				}
			} else {

				Registros.getInstancia().setAH(0xCC); // Falla en la escritura
	...
	public int consultaEstadoDisco() {
		return Registros.getInstancia().getAL();
	}
